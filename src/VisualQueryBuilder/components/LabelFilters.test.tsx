...
describe('LabelFilters', () => {
  ...
  it('renders multiple labels', async () => {
    setup({
      labelsFilters: [
        ...
        { label: 'baz', operator: '!=', value: 'qux' },
        { label: 'quux', operator: '=~', value: 'quuz' },
      ],
    });
  ...
  it('renders multiple values for regex selectors', async () => {
    setup({
      labelsFilters: [
        { label: 'bar', operator: '!~', value: 'baz|bat|bau' },
        { label: 'foo', operator: '!~', value: 'fop|for|fos' },
      ],
    });
    expect(screen.getByText(/bar/)).toBeInTheDocument();
    expect(screen.getByText(/baz/)).toBeInTheDocument();
  ...
  it('adds new label', async () => {
    const { onChange } = setup({ labelsFilters: [{ label: 'foo', operator: '=', value: 'bar' }] });
  ...
  it('renders empty input when labels are deleted from outside ', async () => {
    const { rerender } = setup({ labelsFilters: [{ label: 'foo', operator: '=', value: 'bar' }] });
    expect(screen.getByText(/foo/)).toBeInTheDocument();
    expect(screen.getByText(/bar/)).toBeInTheDocument();
    rerender(
  ...
  it('shows error when filter with empty strings  and label filter is required', async () => {
    setup({ labelsFilters: [{ label: '', operator: '=', value: '' }], labelFilterRequired: true });
