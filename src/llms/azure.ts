...
const AZURE_OPENAI_CHAT_COMPLETIONS_PATH = 'azure/completions';
...
export interface ChatCompletionsRequest {
   ...
   *
   * See Azure OpenAI Studio -> deployments to see all available deployments
   */
  deployment: string;
...
export async function chatCompletions(request: ChatCompletionsRequest): Promise<ChatCompletionsResponse> {
  const response = await getBackendSrv().post<ChatCompletionsResponse>(`${LLM_PLUGIN_ID}/resources/${AZURE_OPENAI_CHAT_COMPLETIONS_PATH}`, request, {
...
export function streamChatCompletions(request: ChatCompletionsRequest): Observable<ChatCompletionsResponse<ChatCompletionsChunk>> {
  const channel: LiveChannelAddress = {
    ...
    path: AZURE_OPENAI_CHAT_COMPLETIONS_PATH,
    data: request,
  };
...
export const enabled = async () => {
  try {
    ...
    if (!loggedWarning) {
      logDebug(String(e));
      logDebug('Failed to check if Azure OpenAI is enabled. This is expected if the Grafana LLM plugin is not installed, and the above error can be ignored.');
      loggedWarning = true;
    }
    return false;
  }
